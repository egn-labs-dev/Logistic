...
# Write secrets to correct paths (KV-v2 auto-prepends /data/)
Invoke-Expression "$env:VAULT_CMD kv put secret/logistics/db url='postgresql+asyncpg://log_admin:CHANGE_ME@db:5432/logistics_secure_db'"
Invoke-Expression "$env:VAULT_CMD kv put secret/logistics/ai gemini_key='YOUR_GEMINI_API_KEY_HERE'"

Invoke-Expression "$env:VAULT_CMD auth enable approle"

# Policy must match the template paths
"path `"secret/data/logistics/*`" { capabilities = [`"read`"] }" | docker-compose -f docker-compose.prod.yml exec -T -e VAULT_ADDR=http://127.0.0.1:8200 vault vault policy write app-policy -
...
Write-Host "Vault initialized successfully! Remember to change default credentials."
